...
$(function () {
  ...
  $(document).on("click", function (e) {
    if (
      ...
      !$(".account, .account *, .profile").is(e.target)
    ) {
      $(".account").toggleClass("active");
    }
  });
  ...
  $("header .search-btn, .mobile-search-header .mobile-search-back").on(
    "click",
    function () {
  ...
  $(document).on("click", ".ripple", function (e) {
    var $ripple = $('<span class="rippling" />'),
      ...
      size = 0,
      animateSize = parseInt(
        Math.max($button.width(), $button.height()) * Math.PI
      );
